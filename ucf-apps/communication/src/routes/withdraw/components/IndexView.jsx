...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.communicationWithdraw.updateState({powerButton:this.props.powerButton});
        actions.communicationWithdraw.updateState({ifPowerBtn:this.props.ifPowerBtn});
        actions.communicationWithdraw.updateState({gridColumn:this.props.gridColumn});
        actions.communicationWithdraw.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.communicationWithdraw.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    };
    ...
    switchToCardView = (obj) =>{
        ...
        actions.communicationWithdraw.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    };
    ...
    switchEdit = () =>{
        ...
        actions.communicationWithdraw.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.communicationWithdraw.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.communicationWithdraw.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.communicationWithdraw.updateState({showModal : true});
    };
    ...
    onSave = () => {
        ...
        actions.communicationWithdraw.updateRowData({'record':_formObj});
        this.switchEdit();
    };
