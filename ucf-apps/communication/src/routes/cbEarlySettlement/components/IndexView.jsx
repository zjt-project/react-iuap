...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.communicationCbEarlySettlement.updateState({powerButton:this.props.powerButton});
        actions.communicationCbEarlySettlement.updateState({ifPowerBtn:this.props.ifPowerBtn});
        actions.communicationCbEarlySettlement.updateState({gridColumn:this.props.gridColumn});
        actions.communicationCbEarlySettlement.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.communicationCbEarlySettlement.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.communicationCbEarlySettlement.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.communicationCbEarlySettlement.updateState({isEdit : !this.state.isEdit});
    }
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.communicationCbEarlySettlement.updateState({formObject:_formObject});
        }else{
            
            //新增完成初始化form表单
            actions.communicationCbEarlySettlement.updateState({formObject:{
                //租赁方式
                lease_method:'0',
                //本金是否开票
                if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.communicationCbEarlySettlement.updateState({showModal : true});      
    }
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {  //查看选中项数据前进行一次单选校验
            this.switchToCardView(param);
            actions.communicationCbEarlySettlement.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        ...
        actions.communicationCbEarlySettlement.updateRowData({'record':_formObj});
        this.switchEdit();
    }
