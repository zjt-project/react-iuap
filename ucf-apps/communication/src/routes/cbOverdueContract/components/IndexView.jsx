...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.communicationCbOverdueContract.updateState({powerButton:this.props.powerButton});
        actions.communicationCbOverdueContract.updateState({ifPowerBtn:this.props.ifPowerBtn});
        actions.communicationCbOverdueContract.updateState({gridColumn:this.props.gridColumn});
        actions.communicationCbOverdueContract.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.communicationCbOverdueContract.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.communicationCbOverdueContract.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.communicationCbOverdueContract.updateState({isEdit : !this.state.isEdit});
    }
    ...
    onalterSearch = () =>{
        ...
        actions.communicationCbOverdueContract.loadList(queryParam);
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.communicationCbOverdueContract.updateState({formObject:_formObject});
        }else{
            
            //新增完成初始化form表单
            actions.communicationCbOverdueContract.updateState({formObject:{
                //租赁方式
                lease_method:'0',
                //本金是否开票
                if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.communicationCbOverdueContract.updateState({showModal : true});      
    }
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {  //查看选中项数据前进行一次单选校验
            this.switchToCardView(param);
            actions.communicationCbOverdueContract.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        ...
        actions.communicationCbOverdueContract.updateRowData({'record':_formObj});
        this.switchEdit();
    }
