...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.communicationCapital.updateState({powerButton:this.props.powerButton});
        actions.communicationCapital.updateState({ifPowerBtn:this.props.ifPowerBtn});
        actions.communicationCapital.updateState({gridColumn:this.props.gridColumn});
        actions.communicationCapital.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.communicationCapital.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    };
    ...
    switchToCardView = (obj) =>{
        ...
        actions.communicationCapital.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    };
    ...
    switchEdit = () =>{
        ...
        actions.communicationCapital.updateState({isEdit : !this.state.isEdit});
    };
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.communicationCapital.updateState({formObject:_formObject});
        }else{

            //新增完成初始化form表单
            actions.communicationCapital.updateState({formObject:{
                    //租赁方式
                    lease_method:'0',
                    //本金是否开票
                    if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.communicationCapital.updateState({showModal : true});
    };
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {  //查看选中项数据前进行一次单选校验
            this.switchToCardView(param);
            actions.communicationCapital.updateState({bt:false});
        });
    };
    ...
    onSave = () => {
        ...
        actions.communicationCapital.updateRowData({'record':_formObj});
        this.switchEdit();
    };
