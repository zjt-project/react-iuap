...
class IndexView extends Component {
    ...
    componentWillMount() {
        actions.communicationCbBadContract.updateState({powerButton:this.props.powerButton});
        actions.communicationCbBadContract.updateState({ifPowerBtn:this.props.ifPowerBtn});
        actions.communicationCbBadContract.updateState({gridColumn:this.props.gridColumn});
        actions.communicationCbBadContract.updateState({ifGridColumn:this.props.ifGridColumn});
    ...
    switchToListView = () =>{
        ...
        actions.communicationCbBadContract.updateState({ formObject : {},isGrid : true,isEdit : false, showForm : false});
    }
    ...
    switchToCardView = (obj) =>{
        ...
        actions.communicationCbBadContract.updateState({ formObject : _formObj,isGrid : false,isEdit : false, showForm : true});
    }
    ...
    switchEdit = () =>{
        ...
        actions.communicationCbBadContract.updateState({isEdit : !this.state.isEdit});
    }
    ...
    onAdd = () =>{
        ...
        if(objectForm){
            ...
            actions.communicationCbBadContract.updateState({formObject:_formObject});
        }else{
            
            //新增完成初始化form表单
            actions.communicationCbBadContract.updateState({formObject:{
                //租赁方式
                lease_method:'0',
                //本金是否开票
                if_corpus_tickets:'0',
        ...
        //填出新增窗口
        actions.communicationCbBadContract.updateState({showModal : true});      
    }
    ...
    onView = () =>{
        singleRecordOper(this.props.selectedList,(param) => {  //查看选中项数据前进行一次单选校验
            this.switchToCardView(param);
            actions.communicationCbBadContract.updateState({bt:false});
        });
    }
    ...
    onSave = () => {
        ...
        actions.communicationCbBadContract.updateRowData({'record':_formObj});
        this.switchEdit();
    }
